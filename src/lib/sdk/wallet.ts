...
// devnet
const connection = new Connection('https://solana-devnet.g.alchemy.com/v2/cuZrbUds5kC5rvZhSgpQ8Dk8bWPRPrtY', 'confirmed')
  ...
  getTokenAccountBalance: async (publicKey: string) => {
    ...
    try {
        const data = (
            await (
                await rayOwner
            ).account.fetchWalletTokenAccounts({ commitment: 'confirmed', forceUpdate: true })
